...
\label{sec:rw-spear}
Targeted attacks --- spear phishing, whaling, and Business Email Compromise --- differ from mass phishing in their
...
\citep{duman2016emailprofiler}, and analysis of relay infrastructure \citep{luo2024characterizing} all catch mail
from compromised legitimate accounts whose content reads as normal. In each case, maliciousness is
determined by who sent a message and in what context, not by what was written.
...
constructor}, a shared list or affiliation, not as a flat feature. Language models then drastically lower the
...
\citep{brown2025web}. The battleground shifts from \emph{style} to \emph{structure}: once content can be
generated perfectly, the defense must read relations.
...
(authority, urgency) reduces detectability \citep{butavicius2022why}. Awareness, decision style, and knowledge
...
TGN-lite is a minimal realization of this paradigm by design, so that the time-shuffle control isolates the
...
\label{sec:rw-gap}
Three gaps emerge from the review. Spear phishing detection remains dominated by the content signal, while the
residual $2\%$ bypasses it by definition, with no unified relational treatment for lateral/BEC and external
impersonation. Graph approaches target the web and transactions rather than the domain knowledge graph of
communication, and organizational susceptibility is not measured structurally. Finally, evaluations rarely control for leaks or test transfer across topologies, so effectiveness tends to be
ill-defined. The present work
addresses all three: a domain knowledge graph, its two detection invariants, a susceptibility metric, and a
